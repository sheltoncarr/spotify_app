...
def get_short_term_track_recs(spotify):
    ...
    artist_list_total = [', '.join(artist['name'] for artist in tracks['artists']) for tracks in rec_results]
...
def get_short_term_artist_recs(spotify):
    ...
    df = pd.DataFrame({'Track': track_list, 'Artist': artist_list_total, 'Genre': genre_list, 'Audio Sample': audio_sample_list})
    df.index += 1
    return df
